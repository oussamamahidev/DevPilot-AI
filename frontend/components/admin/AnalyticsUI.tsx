...
export function ErrorCard({
  ...
  return (
    <section className="mb-6 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">
    ...
    <div className={className}>
      {label ? (
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="font-medium text-slate-500">{formatPercent(width)}</span>
        </div>
      ) : null}
...
export function ProgressRing({
  ...
  return (
    <div className="grid place-items-center gap-2">
      ...
      >
        <div
          className="grid rounded-full bg-white text-center"
      ...
      <div className="flex flex-col gap-5 md:flex-row md:items-center">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-lg font-semibold text-slate-950">Platform Health Score</h3>
        ...
        <div className="relative h-64 min-w-0 sm:h-72">
          ...
          {centerLabel ? (
            <div className="pointer-events-none absolute inset-0 grid place-items-center text-center">
              <div>
                <p className="text-2xl font-semibold text-slate-950">{formatNumber(total)}</p>
                <p className="text-xs font-medium uppercase text-slate-500">{centerLabel}</p>
              </div>
            </div>
          ) : null}
        </div>
          ...
          <div className="grid gap-2">
            {items.slice(0, 8).map((item) => (
              ...
              >
                ...
                <div>
                  <p className="text-xs font-medium uppercase text-slate-500">Final</p>
                  <p className="font-semibold text-slate-950">#{item.final_rank ?? "n/a"}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          return (
            <div key={step.label} className={`relative rounded-lg border p-3 ${toneClasses[tone]}`}>
              ...
              {index < steps.length - 1 ? (
                <span className="absolute right-2 top-2 hidden text-xs text-slate-400 xl:inline">
                  →
                </span>
              ) : null}
            </div>
...
export function AgentTimeline({ runs }: { runs: RagTraceAgentRun[] }) {
  return (
    <section className="min-w-0 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="text-base font-semibold text-slate-950">Agent Timeline</h3>
      {runs.length === 0 ? (
        <div className="mt-5">
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {segments.map((segment) => (
          <span key={segment.label} className="inline-flex items-center gap-2 text-xs text-slate-600">
            <span
